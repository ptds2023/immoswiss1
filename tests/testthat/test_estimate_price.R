...
test_that("estimate_price returns correct results", {
  result <- immoswiss::estimate_price(2, 100, 1004, lausanne)

  expected <- c(2230.70898703841, 1857.27499371082, 2679.22768663987)

  expect_equal(length(result), length(expected))

  for (i in seq_along(result)) {
    expect_equal(result[i], expected[i], tolerance = 1e-8)
  }
})

# Test that an error is thrown for invalid number of rooms
test_that("Invalid number of rooms throws an error", {
  expect_error(immoswiss::estimate_price(3.6, 100, 1000, data),
...
test_that("Invalid location throws an error", {
  expect_error(immoswiss::estimate_price(2, 100, 999, data),
...
test_that("Correct input does not throw an error", {
  expect_no_error(immoswiss::estimate_price(2, 100, 1000, lausanne))
})
